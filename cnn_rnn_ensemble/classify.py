...
def main(config):
    ...
    ## 원핫 벡터 인덱스가 주어지면 단어로 변환, 단어가 주어지면 원핫 벡터 인덱스로 변환
    ## 클래스 명이 주어지면 클래스 인덱스로 변환, 클래스의 인덱스가 주어지면 클래스명으로 변환
    ## 이 매핑 함수가 위 vocab, classes 에 들어가있는 것.
    ...
    with torch.no_grad():
        ...
        for model in ensemble:
            ...
            for idx in range(0, len(lines), config.batch_size):                
                ...
                y_hat += [model(x).cpu()]    
                ## 여기서 y hat 은 (batch size, | class | ) 임.
            # Concatenate the mini-batch wise result
            y_hat = torch.cat(y_hat, dim=0)         ## torch cat 을 통해 미니배치의 결과들을 다 합쳐줌
            ## 그럼 결과적으로 ( N, | class | )
