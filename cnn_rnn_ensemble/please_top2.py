...
def main(config):
    ...
    with torch.no_grad():
        ...
        for i in range(30):
            ...
            #print(label[i], [classes.itos[indice[i][j]] for j in range(config.top_k)], lines[i])
            print(label[i])
        '''    
        total = 0
        for i in range(len(lines)):
            if [label[i]] == [classes.itos[indice[i][1]]]:
                print(classes.itos[indice[i][0]], classes.itos[indice[i][1]])
