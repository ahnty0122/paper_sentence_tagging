...
def main(config):
    ...
    with torch.no_grad():
        ...
        for i in range(len(lines)):
            if ([classes.itos[indice[i][0]]] == ['대상 데이터'] and [classes.itos[indice[i][1]]] == ['데이터처리']):
                index.append(i)
            else:
        ...
        #print(index)            
        count2 = main2(config, index)
        count = count + count2
        print('Accuracy: %f %%' % (100 * count / len(lines)))
             


def main2(config, index):
    saved_data = torch.load(
        './simple_ntc/models/new_ty2.pth',
        map_location='cpu' if config.gpu_id < 0 else 'cuda:%d' % config.gpu_id
    )   
    train_config = saved_data['config']
    rnn_best = saved_data['rnn']
    ...
    with torch.no_grad():
        ...
        for i in range(len(lines)):
            if i in index : 
                if classes.itos[indice[i][0]] == label[i] :
                    count2 += 1
